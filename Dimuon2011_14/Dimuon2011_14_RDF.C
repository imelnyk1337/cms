...
void Dimuon2011_14_RDF() {
    ...
    ROOT::RDF::SaveGraph(df_DoubleMu, "computation_graph_df_DoubleMu.dot");
    ...
    ROOT::RDF::SaveGraph(df_MuOnia, "computation_graph_df_MuOnia.dot");

    // Event loops is run here (once each)
    TH1D h_dimulog1  = *h_dimulog1_ptr;
    // instead we can use h_dimulog1_ptr->Draw() directly
